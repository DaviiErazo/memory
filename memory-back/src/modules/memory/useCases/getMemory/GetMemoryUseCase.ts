...
export class GetMemoryUseCase implements UseCase<GetMemoryRequestDTO, Promise<Response>> {
    ...
    public async execute(req: GetMemoryRequestDTO): Promise<Response> {
        ...
        if (RangeDateOrError.isFailure) {
            return left(Result.fail<any>(RangeDateOrError.error.toString())) as Response;
        }
